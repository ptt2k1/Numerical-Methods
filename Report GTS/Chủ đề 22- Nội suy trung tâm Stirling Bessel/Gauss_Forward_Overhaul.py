...
def nodes(xs:list,c:float)-> list:
    ...
    if abs(xs[0]-c)<abs(c-xs[-1]):
        ...
        k-=1
        if j+k+1 <=4:
            exit(colored('TOO FEW POINTS TO PERFORM GAUSS FORWARD INTERPOLATION','red'))
        elif 4 < j+k+1< 7:
        ...
        if j+k+1 <=4:
            ...
            return range(i-3,i+4)
def Gauss_Forward_Overhaul(xs: list,ys: list,x: float):
    ...
    print(colored(f'Gauss Forward interpolation deduces value at x = {x}: ','yellow'))
...
def main():
    ...
    Gauss_Forward_Overhaul(xs,ys,start)
main()